...
const RealApi = {
    ...
    users: {
        ...
        updateAvatar: async (userId: string, avatar: string) => {
            const response = await request<{ avatar: string, message: string }>('POST', `/users/${userId}/avatar`, { avatar });
            return response;
        }
    },
    departments: {
        getAll: async (): Promise<DepartmentDef[]> => {
            try {
                ...
                const departments = Array.isArray(response) ? response : (response.departments || []);
                return departments;
            } catch (error) {
    ...
    attendance: {
        getTodayStatus: async (userId: string): Promise<AttendanceRecord | null> => {
            try {
                const response = await request<{ success: boolean; data: { lastRecord: any } }>('GET', `/attendance/status`);
                const record = response.data?.lastRecord;
        ...
        getChannels: async (userId: string): Promise<ChatChannel[]> => {
            try {
                ...
                const channels = (response.channels || []).map((ch: any) => {
                    ...
                    console.log('✅ 映射頻道:', mapped.id, '最後訊息:', mapped.lastMessage ? `${mapped.lastMessage.userName}: ${mapped.lastMessage.content}` : '無');
                    return mapped;
                });
                return channels;
            } catch (error) {
