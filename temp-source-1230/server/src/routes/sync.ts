...
router.post('/upload', authenticateToken, async (req, res) => {
  try {
    ...
    // 記錄同步日誌
    await logSystemAction(db, currentUser, 'SYNC_UPLOAD', 
      `上傳 ${results.success.length} 項變更，${results.conflicts.length} 項衝突，${results.errors.length} 項錯誤`);
...
router.post('/resolve', authenticateToken, async (req, res) => {
  try {
    ...
    // 記錄解決日誌
    await logSystemAction(db, currentUser, 'RESOLVE_CONFLICT', 
      `解決 ${table} 記錄 ${recordId} 衝突: ${actionDescription}`);
...
router.post('/retry', authenticateToken, async (req, res) => {
  try {
    ...
    for (const syncId of syncIds) {
      try {
        // 獲取同步項目
        const syncItem = await db.get(
          'SELECT * FROM sync_queue WHERE id = ? AND user_id = ?',
          [syncId, currentUser.id]
        );
