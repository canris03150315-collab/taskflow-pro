...
// DELETE /api/users/:id - ?芷?冽嚗?閬?BOSS ??MANAGER 甈?嚗?
...
// POST /api/users/:id/reset-password - ?蔭?冽撖Ⅳ嚗?閬恣?甈?嚗?
...
router.get('/department/:departmentId', auth_1.authenticateToken, async (req, res) => {
    try {
        const db = req.db;
        const { departmentId } = req.params;
        const users = await db.all('SELECT id, name, role, department, avatar, username, created_at, updated_at FROM users WHERE department = ? ORDER BY role DESC, name ASC', [departmentId]);
        const usersWithPermissions = users.map(user => ({
            ...user,
            permissions: user.permissions ? JSON.parse(user.permissions) : undefined
        }));
...
// POST /api/users/:id/avatar - 銝?剖? (base64)
router.post('/:id/avatar', auth_1.authenticateToken, (0, auth_1.requireSelfOrAdmin)('id'), async (req, res) => {
    try {
        const db = req.db;
        const currentUser = req.user;
        const { id } = req.params;
        ...
        if (!avatar) {
            return res.status(400).json({ error: '瘝????剖??豢?' });
        }

        // 撽? base64 ?澆?
        if (!avatar.startsWith('data:image/')) {
            return res.status(400).json({ error: '?剖??澆??航炊嚗????????? });
        }

        // 瑼Ｘ base64 憭批?嚗???5MB嚗?        const base64Data = avatar.split(',')[1];
        if (base64Data && Buffer.byteLength(base64Data, 'base64') > 5 * 1024 * 1024) {
            return res.status(400).json({ error: '?剖?瑼??之嚗??豢?撠 5MB ???? });
        }
 ...
 // 閮??亥?
 await (0, logger_1.logSystemAction)(db, currentUser, 'UPDATE_AVATAR', '?湔?冽?剖?: ' + id);
