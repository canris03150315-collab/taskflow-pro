...
router.post("/templates", auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      console.log("已更新模板:", template.id, "isDaily:", template.isDaily);
    } else {
      db.run("INSERT INTO routine_templates (id, title, description, frequency, department_id, created_by, created_at, items, read_by, last_updated, is_daily) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
        template.id,
        template.title,
        "",
        "daily",
      ...
      console.log("已新增模板:", template.id, "isDaily:", template.isDaily);
    }
