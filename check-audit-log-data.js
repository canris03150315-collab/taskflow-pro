...
console.log('1. approval_audit_log table (審核歷史記錄):');
try {
  ...
  if (count.count > 0) {
    ...
    samples.forEach((r, i) => {
      ...
      console.log(`      Authorization ID: ${r.authorization_id || 'N/A'}`);
    });
  }
} catch (error) {
...
console.log('\n2. report_authorizations table (審核授權):');
try {
...
console.log('\n=== Check Complete ===');
console.log('\nConclusion:');
console.log('- approval_audit_log: 審核操作的歷史記錄（應該顯示在前端）');
console.log('- report_authorizations: 審核授權的狀態記錄（不是歷史日誌）');
