...
// 1. Get a test KOL
const kol = db.prepare('SELECT id, kol_name FROM kol_profiles LIMIT 1').get();
if (!kol) {
  console.log('❌ No KOL found in database');
  db.close();
  process.exit(1);
}
console.log(`Test KOL: ${kol.kol_name} (${kol.id})`);
...
try {
  ...
  console.log('\nVerification:');
  console.log(`   KOL: ${created.kol_id}`);
