...
router.delete('/channels/:channelId', authenticateToken, async (req, res) => {
    try {
        ...
        // 檢查當前用戶是否在頻道中
        if (!participants.includes(currentUser.id)) {
            return res.status(403).json({ error: '您無權刪除此頻道' });
        }
        ...
        res.json({ success: true, message: '聊天室已刪除' });
    } catch (error) {
        console.error('刪除聊天室失敗:', error);
        res.status(500).json({ error: '刪除聊天室失敗' });
    }
});
