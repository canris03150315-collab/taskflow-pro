...
console.log('=== Attendance Records Comparison (Jan 23-26) ===\n');
...
const backupRecords = backupDb.prepare(`
  SELECT date, user_id, check_in, check_out, status 
...
if (backupRecords.length > 0) {
  ...
  backupRecords.slice(0, 10).forEach((r, i) => {
    console.log(`  ${i + 1}. Date: ${r.date}, User: ${r.user_id}, Check-in: ${r.check_in || 'N/A'}, Check-out: ${r.check_out || 'N/A'}, Status: ${r.status || 'N/A'}`);
  });
}
  ...
  if (missing.length > 0) {
    ...
    Object.keys(missingByDate).sort().forEach(date => {
      ...
      missingByDate[date].forEach((r, i) => {
        console.log(`    ${i + 1}. User: ${r.user_id}, Check-in: ${r.check_in || 'N/A'}, Check-out: ${r.check_out || 'N/A'}, Status: ${r.status || 'N/A'}`);
      });
    });
  }
}
