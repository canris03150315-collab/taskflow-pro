...
if (content.includes(oldGetQuery)) {
    ...
    console.log('⚠️  GET route pattern not found exactly, trying partial match...');
    // Try a more flexible pattern
    const flexPattern = /let reports;\s+if \(currentUser\.role === "EMPLOYEE"\) \{[\s\S]{100,300}reports = await db\.all\("SELECT \* FROM reports ORDER BY created_at DESC LIMIT 50"\);[\s\S]{0,50}\}/;
    if (content.match(flexPattern)) {
        content = content.replace(flexPattern, newGetQuery);
        console.log('✅ GET route modified (partial match)');
    } else {
        console.log('❌ Could not find GET route to modify');
    }
}

// 2. Fix PUT /:id route - Check ownership within 7 days
console.log('\n2. Modifying PUT /:id route...');
// Find PUT route start
const putStart = content.indexOf('router.put("/:id"');
if (putStart > 0) {
    const putEnd = content.indexOf('router.delete', putStart);
    let putSection = content.substring(putStart, putEnd);
    
    // Find where to insert ownership check (after currentUser declaration)
...
const deleteStart = content.indexOf('router.delete("/:id"', putEnd || 0);
if (deleteStart > 0) {
    ...
    if (afterCurrentUserDel > 0) {
        ...
        const isOwnReport = reportToDelete.user_id === currentUser.id;
        const within7Days = reportAgeDays <= 7;
...
console.log('\n=== Fix Complete ===');
console.log('✅ All three routes modified successfully');
console.log('📦 Backup saved at:', backupPath);
console.log('\nNext: Test syntax with Node.js');
