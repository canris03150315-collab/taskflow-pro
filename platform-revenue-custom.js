...
function parseExcelFile(buffer) {
  ...
  for (let row = 4; row <= range.e.r; row++) {
    ...
    platformNames.forEach((platform) => {
      ...
      // 列 1: 日期
      // 列 2-3: 彩票（工资、反点）
      // 列 4-7: 外接游戏（真人AG、棋牌、外接返点、真人私返）
      // 列 8-9: 彩票分红（领取分红、下发分红）
      // 列 10-11: 外接分红（领取分红、下发分红）
      // 列 12: 私返
      // 列 13: 充值
      // 列 14: 提款
      // 列 15: 借款
      ...
      const lotteryAmount = getCell(baseCol + 1) + getCell(baseCol + 2); // 工资 + 反点
      const externalGameAmount = getCell(baseCol + 3) + getCell(baseCol + 4) + getCell(baseCol + 5) + getCell(baseCol + 6); // 真人AG + 棋牌 + 外接返点 + 真人私返
      const lotteryDividend = getCell(baseCol + 7) + getCell(baseCol + 8); // 领取分红 + 下发分红
      const externalDividend = getCell(baseCol + 9) + getCell(baseCol + 10); // 领取分红 + 下发分红
      const privateReturn = getCell(baseCol + 11); // 私返
      const depositAmount = getCell(baseCol + 12); // 充值
      const withdrawalAmount = getCell(baseCol + 13); // 提款
      const loanAmount = getCell(baseCol + 14); // 借款
      const profit = getCell(baseCol + 15); // 营利
      const balance = getCell(baseCol + 16); // 馀额
