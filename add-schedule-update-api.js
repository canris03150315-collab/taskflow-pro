...
console.log('Adding schedule update API...\n');
...
try {
  ...
  const rejectRouteEnd = content.indexOf(match[0]);
  
  // Add update schedule route before rules routes
  const updateRoute = `
  ...
  router.put('/:id', authenticateToken, async (req, res) => {
    try {
      ...
      // Re-check conflicts with updated schedule
      const deptSchedules = await db.all(
        'SELECT * FROM schedules WHERE department_id = ? AND year = ? AND month = ? AND id != ? AND status = "APPROVED"',
        [schedule.department_id, schedule.year, schedule.month, id]
      );
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (onDutyCount < minOnDuty) {
          conflicts.push({
            ...
            onDuty: onDutyCount,
            required: minOnDuty,
            message: \`\${schedule.month}/\${day} only \${onDutyCount} staff on duty (min: \${minOnDuty})\`
          });
        }
      }
  ...
  const before = content.substring(0, rejectRouteEnd);
  const after = content.substring(rejectRouteEnd);
  
  content = before + updateRoute + after;
  
  fs.writeFileSync(filePath, content, 'utf8');
  
  console.log('OK Schedule update API added');
