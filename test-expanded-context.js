...
const attendanceRecords = db.prepare("SELECT user_id, date, status FROM attendance_records WHERE date >= ? ORDER BY date DESC LIMIT 100", [sevenDaysAgo]).all();

const recentWorkLogs = db.prepare('SELECT id, user_id, content, date FROM work_logs ORDER BY date DESC LIMIT 10').all();

const today = new Date().toISOString().split('T')[0];
const routineRecords = db.prepare("SELECT department_id, COUNT(*) as total, SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) as completed FROM routine_records WHERE date = ? GROUP BY department_id", [today]).all();
