...
const sqlcipher_1 = __importDefault(require("sqlcipher"));
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const crypto_1 = __importDefault(require("crypto"));
class Database {
    constructor(dbPath = './data/taskflow.db', encryptionKey) {
        this.db = null;
        this.dbPath = path_1.default.resolve(dbPath);
        this.encryptionKey = encryptionKey || this.generateEncryptionKey();
    }
    generateEncryptionKey() {
        const keyFile = path_1.default.join(path_1.default.dirname(this.dbPath), '.db-key');
        ...
        fs_1.default.chmodSync(keyFile, 0o600); // 僅擁有者可讀寫
        return key;
    }
    async initialize() {
        return new Promise((resolve, reject) => {
            ...
            this.db = new sqlcipher_1.default.Database(this.dbPath, (err) => {
                if (err) {
                    reject(err);
                    return;
                }
    ...
    async insertDefaultDepartments() {
        ...
        for (const dept of defaultDepts) {
            await this.run('INSERT OR IGNORE INTO departments (id, name, theme, icon) VALUES (?, ?, ?, ?)', [dept.id, dept.name, dept.theme, dept.icon]);
        }
    }
    ...
    async getStats() {
        ...
        return {
            ...
            dbSize: fs_1.default.statSync(this.dbPath).size
        };
    }
}
