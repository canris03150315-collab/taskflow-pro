...
router.post('/', authenticateToken, async (req, res) => {
  try {
    const db = req.db;
    const currentUser = req.user;
    const { type, content, reportDate, createdAt } = req.body;
    ...
    await dbCall(db, 'run',
      ...
      [id, type || 'DAILY', currentUser.id, finalCreatedAt, JSON.stringify(content)]
    );
...
// GET /approval/status - Check current authorization status
router.get('/approval/status', authenticateToken, async (req, res) => {
  try {
    ...
    // Check if user has an active (non-expired) authorization
    const auth = raw.prepare(
      `SELECT * FROM report_authorizations
       WHERE requester_id = ? AND status = 'approved' AND expires_at > datetime('now')
    ...
    // Table might not exist yet — return not authorized
    res.json({ isAuthorized: false, authorization: null });
  }
});
...
router.get('/approval/check', authenticateToken, async (req, res) => {
  try {
    ...
    if (auth) {
      res.json({ isAuthorized: true, authorization: auth });
    } else {
...
// IMPORTANT: Export as reportRoutes to match server.js expectations
