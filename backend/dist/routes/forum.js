...
function mapSuggestion(record) {
  ...
  return {
    ...
    statusChangedBy: record.status_changed_by,
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    let suggestions = [];
    try {
      suggestions = dbCall(db, 'prepare', 'SELECT * FROM suggestions ORDER BY created_at DESC').all() || [];
    } catch (tableErr) {
      // Table may not exist on fresh DB - return empty
      console.warn('Suggestions table query failed (may not exist):', tableErr.message);
      suggestions = [];
    }
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (status !== undefined) {
      ...
      values.push(status);
      // ????????????????
      updates.push('status_changed_by = ?');
