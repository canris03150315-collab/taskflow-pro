...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    ...
    if (!hasPermission) { return res.status(403).json({ error: '\u7121\u6b0a\u5275\u5efa\u90e8\u9580' }); }
...
router.put('/:id', auth_1.authenticateToken, async (req, res) => {
    const currentUser = req.user;
    const hasPermission = currentUser.role === 'BOSS' || currentUser.role === 'MANAGER' || (currentUser.permissions && currentUser.permissions.includes('MANAGE_DEPARTMENTS'));
    if (!hasPermission) { return res.status(403).json({ error: '\u7121\u6b0a\u66f4\u65b0\u90e8\u9580' }); }
...
router.delete('/:id', auth_1.authenticateToken, async (req, res) => {
    ...
    if (!hasPermission) { return res.status(403).json({ error: '\u7121\u6b0a\u522a\u9664\u90e8\u9580' }); }
    ...
    try {
        ...
        if (usersInDept && usersInDept.count > 0) {
            return res.status(400).json({ error: '\u7121\u6cd5\u522a\u9664\uff1a\u90e8\u9580\u4e2d\u9084\u6709\u54e1\u5de5' });
        }
