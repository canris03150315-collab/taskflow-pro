...
// 依角色回傳可見範圍（對齊 FinanceView 的 PERSONAL/DEPT/ALL 分層，避免任何員工讀到全公司財務）
async function getRecordsForUser(db, user) {
  const role = user.role;
  if (role === 'BOSS' || role === 'MANAGER') {
    return await db.all('SELECT * FROM finance_records ORDER BY date DESC, created_at DESC');
  }
  if (role === 'SUPERVISOR') {
    // 主管：本部門的部門記錄 + 部門成員的個人記錄（個人記錄的 department_id = 擁有者部門）
    return await db.all(
      'SELECT * FROM finance_records WHERE department_id = ? ORDER BY date DESC, created_at DESC',
      [user.department]
    );
  }
  // EMPLOYEE：只有本部門的部門記錄 + 自己的個人記錄
  return await db.all(
    `SELECT * FROM finance_records
         WHERE (scope = 'DEPARTMENT' AND department_id = ?)
...
async function updateRecord(db, id, updateData) {
  ...
  if (updateData.confirmedBy !== undefined) {
    ...
    params.push(updateData.confirmedBy);
  }
  if (updateData.confirmedAt !== undefined) {
    updates.push('confirmed_at = ?');
    params.push(updateData.confirmedAt);
  }
