...
recent.forEach((log, i) => {
  ...
  const fields = Object.keys(log).filter(k => k !== 'id' && k !== 'user_id' && k !== 'date' && k !== 'created_at');
  fields.forEach(field => {
    if (log[field] && typeof log[field] === 'string') {
      console.log(`   ${field}: ${log[field].substring(0, 50)}${log[field].length > 50 ? '...' : ''}`);
    }
  });
});

// Test 4: Check work logs by date
console.log('\nTest 4: Work logs by date (last 10 days)');
...
// Test 5: Check specific dates (26-29)
console.log('\nTest 5: Work logs for Jan 26-29');
...
dates.forEach(date => {
  ...
  if (count.count > 0) {
    const logs = db.prepare('SELECT * FROM work_logs WHERE date = ? LIMIT 3').all(date);
    logs.forEach(log => {
      const user = db.prepare('SELECT name FROM users WHERE id = ?').get(log.user_id);
...
console.log('\nRecent work logs in backup (first 10):');
recentBackup.slice(0, 10).forEach((log, i) => {
...
if (total.count < totalBackup.count) {
  ...
  console.log('Data loss detected:', totalBackup.count - total.count, 'logs missing');
} else if (total.count > totalBackup.count) {
  console.log('\n✅ Current database has MORE work logs (normal growth)');
  console.log('New logs added:', total.count - totalBackup.count);
} else {
