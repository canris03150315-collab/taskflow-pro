...
Get-Content "migrate-platform-revenue-detailed.cjs" -Raw | ssh ${SERVER} "cat > /tmp/migrate-platform-revenue-detailed.cjs"
Write-Host "✓ 遷移腳本已上傳" -ForegroundColor Green

# 步驟 3: 上傳新的解析器
Write-Host "步驟 3/8: 上傳新的解析器..." -ForegroundColor Yellow
Get-Content "platform-revenue-detailed.js" -Raw | ssh ${SERVER} "cat > /tmp/platform-revenue-detailed.js"
...
Write-Host "1. 測試平台營收功能" -ForegroundColor White
Write-Host "2. 確認詳細欄位正確顯示" -ForegroundColor White
Write-Host "3. Git commit 所有變更" -ForegroundColor White
Write-Host "4. 更新 WORK_LOG_CURRENT.md" -ForegroundColor White
