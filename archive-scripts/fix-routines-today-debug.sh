#!/bin/bash
echo "=== 修復 /api/routines/today (加入除錯) ==="
...
router.get("/templates", auth_1.authenticateToken, async (req, res) => {
  try {
    const db = req.db;
    const templates = await db.all("SELECT * FROM routine_templates ORDER BY last_updated DESC") || [];
...
router.post("/templates", auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    const existing = await db.get("SELECT id FROM routine_templates WHERE id = ?", [template.id]);
    console.log("existing 結果:", existing);
    
    if (existing) {
      await db.run("UPDATE routine_templates SET department_id = ?, title = ?, items = ?, last_updated = ?, is_daily = ? WHERE id = ?", [
      ...
      await db.run("INSERT INTO routine_templates (id, title, description, frequency, department_id, created_by, created_at, items, read_by, last_updated, is_daily) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
        template.id,
        template.title,
        "",
        "daily",
...
router.post("/templates/:id/read", auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    const template = await db.get("SELECT read_by FROM routine_templates WHERE id = ?", [id]);
    if (!template) {
    ...
    if (!readBy.includes(userId)) {
      readBy.push(userId);
      await db.run("UPDATE routine_templates SET read_by = ? WHERE id = ?", [JSON.stringify(readBy), id]);
    }
    res.json({ success: true });
  } catch (error) {
...
router.get("/today", auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    console.log("today API 參數: userId=", userId, "deptId=", deptId, "today=", today);
    
    await db.run("CREATE TABLE IF NOT EXISTS routine_records (id TEXT PRIMARY KEY, template_id TEXT NOT NULL, user_id TEXT NOT NULL, date TEXT NOT NULL, items TEXT DEFAULT '[]', completed_at TEXT)");
    
    const dailyTemplate = await db.get("SELECT * FROM routine_templates WHERE department_id = ? AND is_daily = 1 LIMIT 1", [deptId]);
    console.log("dailyTemplate 查詢結果:", dailyTemplate ? dailyTemplate.title : "null");
    
    if (!dailyTemplate) {
      console.log("沒有找到每日任務模板 for deptId:", deptId);
      return res.json(null);
    }
    
    let record = await db.get("SELECT * FROM routine_records WHERE template_id = ? AND user_id = ? AND date = ?", [dailyTemplate.id, userId, today]);
    console.log("record 查詢結果:", record ? record.id : "null");
    
    if (!record) {
      const templateItems = JSON.parse(dailyTemplate.items || "[]");
      const recordItems = templateItems.map((text) => ({ text, completed: false }));
      const recordId = "record-" + Date.now();
      await db.run("INSERT INTO routine_records (id, template_id, user_id, date, items) VALUES (?, ?, ?, ?, ?)", [recordId, dailyTemplate.id, userId, today, JSON.stringify(recordItems)]);
      record = { id: recordId, template_id: dailyTemplate.id, user_id: userId, date: today, items: JSON.stringify(recordItems) };
      console.log("建立新 record:", recordId);
    }
...
router.post("/records/:id/toggle", auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    const record = await db.get("SELECT * FROM routine_records WHERE id = ?", [id]);
    if (!record) {
    ...
    await db.run("UPDATE routine_records SET items = ?, completed_at = ? WHERE id = ?", [JSON.stringify(items), completedAt, id]);
    res.json({ success: true });
  } catch (error) {
