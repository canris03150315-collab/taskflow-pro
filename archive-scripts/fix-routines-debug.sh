...
router.post("/templates", auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    let existing = null;
    try {
      existing = db.get("SELECT id FROM routine_templates WHERE id = ?", [template.id]);
      console.log("existing 結果:", existing);
    } catch (e) {
      console.error("查詢existing錯誤:", e.message);
    }
    
    if (existing) {
      try {
        const result = db.run("UPDATE routine_templates SET department_id = ?, title = ?, items = ?, last_updated = ?, is_daily = ? WHERE id = ?", [
        ...
        console.error("UPDATE錯誤:", e.message);
      }
    } else {
      try {
        const result = db.run("INSERT INTO routine_templates (id, title, description, frequency, department_id, created_by, created_at, items, read_by, last_updated, is_daily) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
          template.id,
          template.title,
          "",
          "daily",
        ...
        console.error("INSERT錯誤:", e.message);
      }
    }
...
router.get("/today", auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    db.run("CREATE TABLE IF NOT EXISTS routine_records (id TEXT PRIMARY KEY, template_id TEXT NOT NULL, user_id TEXT NOT NULL, date TEXT NOT NULL, items TEXT DEFAULT \"[]\", completed_at TEXT)");
    ...
    if (!record) {
      ...
      db.run("INSERT INTO routine_records (id, template_id, user_id, date, items) VALUES (?, ?, ?, ?, ?)", [recordId, dailyTemplate.id, userId, today, JSON.stringify(recordItems)]);
