...
// 找到並移除錯誤的 change-password 路由
const routeStart = content.indexOf("// POST /api/users/:id/change-password");
const routeEnd = content.indexOf("// GET /api/users/department/:departmentId", routeStart);

if (routeStart !== -1 && routeEnd !== -1) {
  content = content.slice(0, routeStart) + content.slice(routeEnd);
  console.log('Removed broken change-password route');
}
...
console.log('SUCCESS: Fixed change-password route');
