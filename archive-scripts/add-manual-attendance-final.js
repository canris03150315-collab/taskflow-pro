...
console.log('Adding manual attendance route with CORRECT values...');
...
// The new route with:
// 1. Correct dbCall syntax: await dbCall(db, 'get/run', sql, [params])
// 2. Correct status value: 'OFFLINE' (not 'completed')
// 3. Unicode escape for Chinese characters
const newRoute = `
...
router.post('/manual', authenticateToken, async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      await dbCall(db, 'run', 
        'UPDATE attendance_records SET clock_out = ?, duration_minutes = ?, work_hours = ?, is_manual = ?, manual_by = ?, manual_reason = ?, manual_at = ? WHERE id = ?',
        [date + 'T' + clockOut, durationMinutes, workHours, 1, currentUser.id, reason, now, existing.id]
      );
...
console.log('SUCCESS: Added manual attendance route with correct status value (OFFLINE)');
