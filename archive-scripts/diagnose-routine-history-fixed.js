...
[twoDaysAgoStr, oneDayAgoStr].forEach(dateStr => {
  ...
  records.forEach(r => {
    ...
    try {
      ...
      console.log(`  WARNING: User ${r.user_id}: Cannot parse completed_items`);
      console.log(`  Raw data: ${r.completed_items}`);
      return;
    }
...
// 7. Check if API returns correct field name
console.log('\n7. Check API field mapping:');
console.log('  Database column: completed_items');
console.log('  Frontend expects: items');
console.log('  Need to check if backend maps completed_items -> items');
