...
// GET /api/users - ?脣??冽?”嚗?閬恣?甈?嚗?router.get('/', auth_1.authenticateToken, (0, auth_1.requireRole)([types_1.Role.BOSS, types_1.Role.MANAGER, types_1.Role.SUPERVISOR]), async (req, res) => {
    try {
        ...
        let params = [];
        // SUPERVISOR ?芾??芸楛?券????        if (currentUser.role === types_1.Role.SUPERVISOR) {
...
// POST /api/users - ?萄遣?啁?塚??閬恣?甈?嚗?router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)([types_1.Role.BOSS, types_1.Role.MANAGER]), async (req, res) => {
    try {
        const db = req.db;
        const currentUser = req.user;
...
// DELETE /api/users/:id - ?芷?冽嚗?閬?BOSS ??MANAGER 甈?嚗?router.delete('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)([types_1.Role.BOSS, types_1.Role.MANAGER]), async (req, res) => {
    try {
        const db = req.db;
        const currentUser = req.user;
        const { id } = req.params;
