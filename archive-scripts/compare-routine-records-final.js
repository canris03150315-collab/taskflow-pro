...
const backupRecords = backupDb.prepare(`
  SELECT id, user_id, department_id, date, task_id, status, completed_at
...
if (backupRecords.length > 0) {
  ...
  backupRecords.slice(0, 5).forEach((r, i) => {
    console.log(`  ${i + 1}. Date: ${r.date}, User: ${r.user_id}, Task: ${r.task_id}, Status: ${r.status}`);
  });
}
  ...
  if (missing.length > 0) {
    ...
    Object.keys(missingByDate).sort().forEach(date => {
      ...
      missingByDate[date].slice(0, 10).forEach((r, i) => {
        console.log(`    ${i + 1}. User: ${r.user_id}, Dept: ${r.department_id}, Task: ${r.task_id}, Status: ${r.status}`);
      });
