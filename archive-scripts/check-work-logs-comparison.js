...
if (backupLogs.length > 0) {
  ...
  backupLogs.slice(0, 5).forEach((r, i) => {
    const contentPreview = r.content ? r.content.substring(0, 50) + '...' : 'N/A';
    console.log(`  ${i + 1}. Date: ${r.date}, User: ${r.user_id}, Content: ${contentPreview}`);
  });
}
  ...
  if (missing.length > 0) {
    ...
    Object.keys(missingByDate).sort().forEach(date => {
      ...
      missingByDate[date].forEach((r, i) => {
        const contentPreview = r.content ? r.content.substring(0, 80) : 'N/A';
