#!/bin/bash
# 修復 routines API - 新增每日任務功能

echo "=== 開始修復 routines API ==="

# 1. 創建完整的 routines.js 路由檔案
  ...
  try {
    ...
    try {
      ...
      if (dbError.message && dbError.message.includes("no such table")) {
        db.run("CREATE TABLE IF NOT EXISTS routine_templates (id TEXT PRIMARY KEY, department_id TEXT NOT NULL, title TEXT NOT NULL, items TEXT DEFAULT \"[]\", last_updated TEXT, read_by TEXT DEFAULT \"[]\", is_daily INTEGER DEFAULT 0)");
        res.json([]);
      } else {
        throw dbError;
      }
...
# 4. 驗證
echo "=== 驗證 ==="
echo "檢查 routines.js 是否存在:"
docker exec taskflow-pro ls -la /app/dist/routes/routines.js
