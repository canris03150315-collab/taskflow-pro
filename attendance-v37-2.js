...
console.log('[Attendance V37.2] Module loading (Pure ASCII)...');
...
const dbCall = async (db, method, sql, params = []) => {
    ...
    try {
        ...
        console.error(`[Attendance V37.2] DB Error (${method}):`, err.message, 'SQL:', sql);
        throw err;
    }
...
router.post('/clock-in', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`[Attendance V37.2] Clock-in attempt by ${currentUser.name} (${currentUser.id}) for date ${today}`);
        ...
        try {
            ...
            if (staleRecords.length > 0) {
                console.log(`[Attendance V37.2] Closing ${staleRecords.length} stale records for ${currentUser.name}`);
            ...
            console.error('[Attendance V37.2] Stale cleanup error:', staleErr.message);
        }
        ...
        if (activeRecord) {
            console.log(`[Attendance V37.2] Already clocked in today for ${currentUser.name}`);
        ...
        console.log(`[Attendance V37.2] Clock-in success for ${currentUser.name}`);
        res.json({
            success: true,
        ...
        console.error('[Attendance V37.2] Clock-in error:', error.message);
...
router.post('/clock-out', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`[Attendance V37.2] Clock-out attempt by ${currentUser.name}`);
        ...
        if (!activeRecord) {
            console.log(`[Attendance V37.2] No active record found for ${currentUser.name}`);
        ...
        console.log(`[Attendance V37.2] Clock-out success for ${currentUser.name}, duration: ${durationMinutes} mins`);
        res.json({
            success: true,
        ...
        console.error('[Attendance V37.2] Clock-out error:', error.message);
...
router.get('/status', authenticateToken, async (req, res) => {
    try {
        ...
        console.error('[Attendance V37.2] Status error:', error.message);
        res.status(500).json({ success: false, error: 'Failed to fetch status' });
    }
});

exports.attendanceRoutes = router;
console.log('[Attendance V37.2] Module export complete. Exported as attendanceRoutes.');
