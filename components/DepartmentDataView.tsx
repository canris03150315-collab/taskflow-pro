...
export const DepartmentDataView: React.FC<DepartmentDataViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'ATTENDANCE' | 'REPORTS' | 'TASKS' | 'PERFORMANCE' | 'FINANCE'
  >('ATTENDANCE');
  ...
  // ESC to close edit attendance modal
  useEffect(() => {
  ...
  const displayReports = reports
    .filter((r) => targetUserIds.includes(r.userId) && filterByDate(r.createdAt))
  ...
  const displayTasks = tasks
    .filter(
      (t) =>
        (targetUserIds.includes(t.assignedToUserId || '') ||
          targetUserIds.includes(t.acceptedByUserId || '')) &&
        filterByDate(t.createdAt.split('T')[0])
    )
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));

  // 4. Performance Data (Roughly filter by period matching start date month)
  const displayPerformance = performanceData.filter((r) => targetUserIds.includes(r.targetUserId));

  // 5. Finance Data
  const displayFinance = financeRecords
    .filter((r) => {
      ...
      if (r.scope === 'PERSONAL' && r.ownerId && targetUserIds.includes(r.ownerId))
  ...
  const handleExport = () => {
    ...
    let data: any[] = [];
    const filename = `export_${activeTab.toLowerCase()}_${startDate}_${endDate}.xlsx`;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full md:w-auto">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <input
              ...
              className="flex-1 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none"
            />
      ...
      {/* TODO: migrate emoji icons to an icon component map (e.g. lucide-react) for consistent styling */}
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm min-h-[400px]">
        ...
        {!isLoading && (
          <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  <div className="bg-white rounded-lg p-3 shadow-sm border border-slate-100">
                    ...
                    <div className="text-xl font-bold text-blue-600">
                      {attendanceSummary.totalDays} <span className="text-sm font-normal">天</span>
                    </div>
                  </div>
                  <div className="bg-white rounded-lg p-3 shadow-sm border border-slate-100">
                    <div className="text-xs text-slate-500 mb-1">總工時</div>
                    <div className="text-xl font-bold text-emerald-600">
                      {attendanceSummary.totalHours}h {attendanceSummary.totalMins}m
                    </div>
                  </div>
                  <div className="bg-white rounded-lg p-3 shadow-sm border border-slate-100">
                    <div className="text-xs text-slate-500 mb-1">平均工時</div>
                    <div className="text-xl font-bold text-indigo-600">
                      {attendanceSummary.avgHours}h {attendanceSummary.avgMins}m
                    </div>
                  </div>
                  <div className="bg-white rounded-lg p-3 shadow-sm border border-slate-100">
                    <div className="text-xs text-slate-500 mb-1">線上打卡</div>
                    <div className="text-xl font-bold text-emerald-500">
                      {attendanceSummary.onlineCount}{' '}
                      <span className="text-sm font-normal">次</span>
                    </div>
                  </div>
                  <div className="bg-white rounded-lg p-3 shadow-sm border border-slate-100">
                    <div className="text-xs text-slate-500 mb-1">離線打卡</div>
                    <div className="text-xl font-bold text-slate-500">
                      {attendanceSummary.offlineCount}{' '}
                      <span className="text-sm font-normal">次</span>
                    </div>
                  </div>
                  <div className="bg-white rounded-lg p-3 shadow-sm border border-slate-100">
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase whitespace-nowrap">
                  ...
                  {activeTab === 'REPORTS' && (
                    <tr>
                      <th className="p-4">日期</th>
                      <th className="p-4">姓名</th>
                      <th className="p-4 text-right">充值</th>
                      <th className="p-4 text-right">提現</th>
                      <th className="p-4 text-right">淨入</th>
                      <th className="p-4 text-right">註冊</th>
                      <th className="p-4">備註</th>
                    </tr>
                  )}
                  {activeTab === 'TASKS' && (
                    <tr>
                      ...
                      <th className="p-4">負責人</th>
                      <th className="p-4">優先級</th>
                  ...
                  {activeTab === 'PERFORMANCE' && (
                    <tr>
                      ...
                      <th className="p-4">姓名</th>
                      <th className="p-4">部門</th>
                      <th className="p-4">總分</th>
                      <th className="p-4">評級</th>
                      <th className="p-4">狀態</th>
                    </tr>
                  )}
                </thead>
                <tbody className="divide-y divide-slate-100 text-sm">
                  {activeTab === 'ATTENDANCE' &&
                    displayAttendance.map((r) => (
                      <tr key={r.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-4">
                          <span
                            className={`px-2 py-0.5 rounded text-xs font-bold ${r.status === 'ONLINE' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}
                          >
                            {r.status}
                          </span>
                        </td>
                        <td className="p-4 flex gap-1 items-center">
                          ...
                          {canEditAttendance && (
                            <button
                              ...
                              className="px-2 py-0.5 rounded text-xs font-bold bg-blue-100 text-blue-600 hover:bg-blue-200"
                            >
                              編輯
                            </button>
                          )}
                          {canManualAttendance && (
                            <button
                              ...
                              className="px-2 py-0.5 rounded text-xs font-bold bg-red-100 text-red-600 hover:bg-red-200"
                            >
                              刪除
                            </button>
                          )}
                    ...
                    displayReports.map((r) => (
                      <tr key={r.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-4 text-right font-mono text-slate-600">
                          {(r.content.depositAmount ?? 0).toLocaleString()}
                        </td>
                        <td className="p-4 text-right font-mono text-red-400">
                          {(r.content.withdrawalAmount ?? 0).toLocaleString()}
                        </td>
                        <td
                          className={`p-4 text-right font-mono font-bold ${(r.content.netIncome ?? 0) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                        >
                    ...
                    displayTasks.map((t) => (
                      <tr key={t.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-4 text-blue-600">
                          {getUserName(t.assignedToUserId || t.acceptedByUserId)}
                        </td>
                        <td className="p-4 text-xs">
                          <span className="border px-1 rounded">{t.urgency}</span>
                        </td>
                  ...
                  {activeTab === 'FINANCE' &&
                    displayFinance.map((r) => (
                      <tr key={r.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-4 text-xs text-slate-400">
                          {r.scope === 'DEPARTMENT' ? '部門公費' : getUserName(r.ownerId)}
                        </td>
                        <td
                          className={`p-4 text-right font-mono font-bold ${r.type === 'INCOME' ? 'text-emerald-600' : 'text-red-500'}`}
                        >
                          {r.type === 'INCOME' ? '+' : '-'}
                  ...
                  {activeTab === 'PERFORMANCE' &&
                    displayPerformance.map((r) => (
                      <tr key={r.id} className="hover:bg-slate-50">
                        <td className="p-4 text-slate-500">{r.period}</td>
                        <td className="p-4 font-bold text-slate-700">
                          {getUserName(r.targetUserId)}
                        </td>
                        <td className="p-4 text-xs text-slate-400">
                          {getDeptName(getUserDept(r.targetUserId))}
                        </td>
                ...
                displayAttendance.map((r) => (
                  <div key={r.id} className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm bg-white p-2 rounded border border-slate-100">
                      <div>
                        <span className="text-slate-400 text-xs block">上班</span>{' '}
                        {new Date(r.clockIn).toLocaleTimeString([], {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      ...
                      <div>
                        <span className="text-slate-400 text-xs block">下班</span>{' '}
                        {r.clockOut
                          ? new Date(r.clockOut).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                  ...
                  <div key={r.id} className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                    ...
                    <div className="bg-white p-3 rounded-lg border border-slate-100 mb-2">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span
                          className={`font-mono font-bold text-lg ${(r.content.netIncome ?? 0) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                        >
                          ${(r.content.netIncome ?? 0).toLocaleString()}
                        </span>
                      </div>
                ...
                displayTasks.map((t) => (
                  <div key={t.id} className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-xs font-bold bg-slate-200 px-2 py-0.5 rounded text-slate-600">
                        {t.status}
                      </span>
                    </div>
              ...
              {activeTab === 'FINANCE' &&
                displayFinance.map((r) => (
                  <div key={r.id} className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                    ...
                    <div className="flex justify-between items-center mb-2">
                      <div className="font-medium text-slate-700 truncate mr-2">
                        {r.description}
                      </div>
                      <div
                        className={`font-mono font-bold text-lg ${r.type === 'INCOME' ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {r.type === 'INCOME' ? '+' : '-'}
                        {(r.amount ?? 0).toLocaleString()}
                      </div>
            ...
            {/* Empty States */}
            {activeTab === 'ATTENDANCE' && displayAttendance.length === 0 && (
              <div className="p-8 text-center text-slate-400">無出勤資料</div>
            )}
            {activeTab === 'REPORTS' && displayReports.length === 0 && (
              <div className="p-8 text-center text-slate-400">無報表資料</div>
            )}
            {activeTab === 'TASKS' && displayTasks.length === 0 && (
              <div className="p-8 text-center text-slate-400">無任務資料</div>
            )}
            {activeTab === 'FINANCE' && displayFinance.length === 0 && (
              <div className="p-8 text-center text-slate-400">無財務資料</div>
            )}
            {activeTab === 'PERFORMANCE' && displayPerformance.length === 0 && (
              <div className="p-8 text-center text-slate-400">無績效資料</div>
            )}
          </>
        )}
      </div>
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={getUserName(editingRecord.userId)}
                  disabled
              ...
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">
                  上班日期 <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
                  value={editDate}
              ...
              {editClockOut && (
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">
                    下班日期 <span className="text-xs text-slate-400">(跨日時修改)</span>
                  </label>
                  <input
                    type="date"
