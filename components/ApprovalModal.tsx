...
export const ApprovalModal: React.FC<ApprovalModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent, action: 'approve' | 'reject' = 'approve') => {
    e.preventDefault();

    if (reason.length < 10) {
      toast.error(action === 'approve' ? '審核原因至少需要 10 個字' : '拒絕原因至少需要 10 個字');
      return;
    }
    ...
    try {
      if (mode === 'request') {
        ...
        if (action === 'approve') {
          ...
          const response = await api.reports.approval.reject(pendingAuthId!, reason);
          toast.success(`已拒絕 ${response.requesterName} 的查看申請`);
          onSuccess(false); // Rejection doesn't grant authorization
        }
      }
