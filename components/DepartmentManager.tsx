...
export const DepartmentManager: React.FC<DepartmentManagerProps> = ({
  ...
  const DepartmentTreeNode: React.FC<{
    ...
    return (
      <div className="relative">
        <div
          className={`bg-white rounded-xl border p-4 shadow-sm group transition ${isEditing ? 'border-blue-500 ring-2 ring-blue-200' : 'border-slate-200 hover:border-slate-300'} ${level > 0 ? 'ml-8' : ''}`}
        >
          {/* 樹狀連接線 */}
          {level > 0 && (
            <>
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3 flex-1">
              <div
                className={`w-12 h-12 rounded-lg flex items-center justify-center text-2xl shadow-sm bg-${dept.theme}-50 text-${dept.theme}-600 flex-shrink-0`}
              >
                {dept.icon}
              </div>
            ...
            {!isUnassigned && (
              <div className="flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100 transition-opacity flex-shrink-0">
                <button
                ...
                {memberCount === 0 && !hasChildren && (
                  <button
                    onClick={async () => {
                      if (await showConfirm(`確定要刪除「${dept.name}」嗎？`)) onDelete(dept.id);
                    }}
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* Add/Edit Form */}
      <div
        className={`border rounded-xl p-6 transition-all ${editingId ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-200'}`}
      >
        <h3
          className={`text-lg font-bold mb-4 flex items-center gap-2 ${editingId ? 'text-blue-800' : 'text-slate-800'}`}
        >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="flex-1 w-full">
            ...
            >
              ...
              {getAvailableParentDepts()
                .filter((d) => d.id !== UNASSIGNED_DEPT_ID)
                .map((d) => (
                  <option key={d.id} value={d.id}>
                    {d.level > 0 ? '　'.repeat(d.level) + '└─ ' : ''}
                    {d.name}
                  </option>
                ))}
            </select>
