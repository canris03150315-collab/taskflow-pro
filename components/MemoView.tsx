...
export const MemoView: React.FC<MemoViewProps> = ({ currentUser }) => {
  ...
  const saveEditTodo = async () => {
    ...
    const updatedMemos = memos.map(m => {
      if (m.id === editingTodo.memoId && m.todos) {
        return {
          ...m,
          todos: m.todos.map(t => t.id === editingTodo.todoId ? { ...t, text: editingText.trim() } : t)
        };
      }
      return m;
    });
  ...
  const colors: { id: Memo['color'], class: string, border: string, ring: string }[] = [
    { id: 'yellow', class: 'bg-yellow-100', border: 'border-yellow-200', ring: 'focus:ring-yellow-400' },
    { id: 'blue', class: 'bg-blue-100', border: 'border-blue-200', ring: 'focus:ring-blue-400' },
    { id: 'green', class: 'bg-emerald-100', border: 'border-emerald-200', ring: 'focus:ring-emerald-400' },
    { id: 'rose', class: 'bg-rose-100', border: 'border-rose-200', ring: 'focus:ring-rose-400' },
    { id: 'purple', class: 'bg-purple-100', border: 'border-purple-200', ring: 'focus:ring-purple-400' },
  ];
         ...
         {createType === 'TEXT' ? (
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-400 ml-auto">💡 提示：可在下方便條卡片內繼續追加項目</span>
                </div>
            </div>
         )}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           {memos.map(memo => {
              ...
              const completedCount = isChecklist ? (memo.todos?.filter(t => t.isCompleted).length || 0) : 0;
              const totalCount = isChecklist ? (memo.todos?.length || 0) : 0;
              const progress = totalCount > 0 ? Math.round((completedCount / totalCount) * 100) : 0;

              return (
                 <div key={memo.id} className={`${colorTheme.class} ${colorTheme.border} border p-5 rounded-xl shadow-sm relative group hover:shadow-md transition duration-300 min-h-[200px] flex flex-col`}>
                        ...
                        {isChecklist ? (
                            <div>
                                ...
                                <div className="h-1.5 w-full bg-black/5 rounded-full mb-3 overflow-hidden">
                                    <div className="h-full bg-slate-600/50 transition-all duration-500" style={{ width: `${progress}%` }}></div>
                                </div>
                                <ul className="space-y-1.5">
                                    {memo.todos?.map(todo => (
                                        ...
                                        >
                                            ...
                                            >
                                                {todo.isCompleted && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>}
                                            </button>
                                            ...
                                            {/* Delete Button */}
                                            <button 
                                              onClick={(e) => { e.stopPropagation(); handleDeleteTodo(memo.id, todo.id); }}
                                ...
                                {/* 在便條內新增項目 */}
                                <form 
                                  onSubmit={(e) => { e.preventDefault(); handleAddTodoToMemo(memo.id); }}
                                ...
                                >
                                  <div className="flex gap-1.5">
                                    <input
                                      ...
                                      onChange={(e) => setMemoInputs({ ...memoInputs, [memo.id]: e.target.value })}
                                      placeholder="新增項目..."
