...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const myPendingTasks = useMemo(
    () =>
      Array.isArray(tasks)
        ? tasks.filter(
            (t) => t.assignedToUserId === currentUser.id && t.status === TaskStatus.ASSIGNED
          )
        : [],
    [tasks, currentUser.id]
  );
    ...
    <div className="max-w-7xl mx-auto space-y-3 md:space-y-6 pb-20 animate-fade-in px-2 md:px-4 lg:px-0">
          ...
          <div className="bg-white px-4 py-2.5 rounded-xl border border-slate-200 shadow-sm text-center flex-shrink-0 min-w-[90px] md:min-w-[100px]">
            ...
            <div className="text-xl md:text-2xl font-black text-slate-800 mt-1">
              {myActiveTasks.length +
                ...
                myCreatedTasks.filter(
                  (t) =>
                    !myActiveTasks.find((a) => a.id === t.id) &&
                    !myPendingTasks.find((p) => p.id === t.id)
                ).length}
            </div>
          </div>
          <div className="bg-white px-4 py-2.5 rounded-xl border border-slate-200 shadow-sm text-center flex-shrink-0 min-w-[90px] md:min-w-[100px]">
            ...
            <div
              className={`text-xl md:text-2xl font-black mt-1 ${unreadAnnouncements.length > 0 ? 'text-red-500' : 'text-slate-800'}`}
            >
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-2 px-2 md:mx-0 md:px-0">
        ...
        >
          ...
          <span className="whitespace-nowrap">新增任務</span>
        </button>
        <button
        ...
        >
          ...
          <span className="whitespace-nowrap">任務列表</span>
        </button>
        <button
        ...
        >
          <span className="text-base md:text-sm">📖</span>{' '}
          <span className="whitespace-nowrap">SOP 文檔</span>
        </button>
        <button
        ...
        >
          ...
          <span className="whitespace-nowrap">部門資料</span>
        </button>
        <button
            ...
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800 flex items-center gap-2">
                <svg className="w-5 h-5 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
                  ...
                  <path
                    fillRule="evenodd"
                    d="M5.403 4.443a.75.75 0 01-.295 1.019l-1.703.94a.75.75 0 010 1.196l1.703.94a.75.75 0 11-.724 1.314l-2.5-1.38a.75.75 0 010-1.314l2.5-1.38a.75.75 0 011.019.295zm9.194 0a.75.75 0 011.018-.295l2.5 1.38a.75.75 0 010 1.314l-2.5 1.38a.75.75 0 11-.724-1.314l1.703-.94-1.703-.94a.75.75 0 01-.294-1.019z"
                    clipRule="evenodd"
                  />
                  <path
                    fillRule="evenodd"
                    d="M10 7.5a2.5 2.5 0 100 5 2.5 2.5 0 000-5z"
                    clipRule="evenodd"
                  />
                </svg>
                我的任務概況
            ...
            <div className="p-4 md:p-5">
              ...
              myCreatedTasks.length === 0 ? (
                <div className="text-center py-8 md:py-12 animate-fade-in">
                  ...
                  <p className="text-sm md:text-base text-slate-400 font-bold mb-1 md:mb-2">
                    目前沒有進行中的任務
                  </p>
                  <p className="text-xs text-slate-300 mb-3 md:mb-4">
                    去任務大廳看看有沒有可以接取的任務吧！
                  </p>
                  <button
                    onClick={() => onChangePage('tasks')}
                ...
                <div className="space-y-2.5 md:space-y-3">
                  {myPendingTasks.slice(0, 5).map((t, index) => (
                    ...
                    >
                      ...
                      <div
                        className="flex-1 font-bold text-sm md:text-base text-slate-700 truncate cursor-pointer min-h-[44px] flex items-center"
                        onClick={() => onChangePage('tasks')}
                      >
                        {t.title}
                      </div>
                  ...
                  {myActiveTasks.slice(0, 5).map((t, index) => (
                    ...
                    >
                      <Badge type="urgency" value={t.urgency} />
                      <div className="flex-1 font-bold text-sm md:text-base text-slate-700 truncate">
                        {t.title}
                      </div>
                  ...
                  {myCreatedTasks
                    ...
                    .map((t) => (
                      <div
                        key={t.id}
                        className="flex items-center gap-2 md:gap-3 p-3 md:p-3.5 rounded-xl border border-slate-200 bg-purple-50/30 hover:bg-purple-50 active:bg-purple-50 transition cursor-pointer min-h-[60px] md:min-h-0"
                        onClick={() => onChangePage('tasks')}
                      >
                        <Badge type="urgency" value={t.urgency} />
                        <div className="flex-1 font-bold text-sm md:text-base text-slate-700 truncate">
                        ...
                        <span className="text-xs font-bold text-purple-600 px-2 py-1 bg-purple-100 rounded whitespace-nowrap">
                          我創建
                        </span>
                      </div>
                    ))}
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        >
                          ...
                          <h4
                            className={`text-sm font-bold flex-1 ${isUnread ? 'text-slate-900' : 'text-slate-500'}`}
                          >
                            {ann.title}
                          </h4>
                        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-4">
              ...
              {activityFeed.length === 0 && (
                <div className="text-center text-slate-400 text-xs py-2">無近期動態</div>
              )}
            </div>
          </div>
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 text-white shadow-xl shadow-slate-200">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  ...
                  <span
                    className={`text-2xl font-black font-mono ${latestReport.content.netIncome >= 0 ? 'text-emerald-400' : 'text-red-400'}`}
                  >
                    {(latestReport.content.netIncome ?? 0) >= 0 ? '+' : ''}
                    {(latestReport.content.netIncome ?? 0).toLocaleString()}
                  </span>
                </div>
                ...
                <button
                  ...
                  className="w-full py-2 bg-white/10 hover:bg-white/20 rounded-lg text-xs font-bold transition mt-2"
                >
                  查看完整報表
                </button>
              </div>
