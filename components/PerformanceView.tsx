...
export const PerformanceView: React.FC<PerformanceViewProps> = ({
  ...
  const handleSaveReview = async () => {
    ...
    try {
      // Calculate score
      const metricsScore =
        ((editingReview.metrics.taskCompletionRate +
          editingReview.metrics.sopCompletionRate +
          editingReview.metrics.attendanceRate) /
          3) *
        0.6;
      const managerScore =
        ((editingReview.ratingWorkAttitude +
          editingReview.ratingProfessionalism +
          editingReview.ratingTeamwork) /
          15) *
        100 *
        0.4;
      ...
      <div className="flex justify-between items-end border-b border-slate-200 pb-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
            ...
            >
              <path
                fillRule="evenodd"
                d="M3.75 2A1.75 1.75 0 002 3.75v.5C2 5.769 2.231 7.5 6 7.5h.345A4.001 4.001 0 009 10.917V13h-.5a2.5 2.5 0 00-2.5 2.5v.75c0 .414.336.75.75.75h6.5a.75.75 0 00.75-.75v-.75a2.5 2.5 0 00-2.5-2.5H11v-2.083A4.001 4.001 0 0013.655 7.5H14c3.769 0 4-1.731 4-3.25v-.5A1.75 1.75 0 0016.25 2H3.75zM6 6c-1.5 0-2.5-.25-2.5-1.75v-.5a.25.25 0 01.25-.25h12.5a.25.25 0 01.25.25v.5C16.5 5.75 15.5 6 14 6H6z"
                clipRule="evenodd"
              />
            </svg>{' '}
            績效考核
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="p-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none"
          />
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingReview({
                        ...editingReview,
                        ratingWorkAttitude: Number(e.target.value),
                      })
                    }
                    className="w-full"
                  />
                  <div className="text-right text-sm font-bold text-blue-600">
                    {editingReview.ratingWorkAttitude} 分
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingReview({
                        ...editingReview,
                        ratingProfessionalism: Number(e.target.value),
                      })
                    }
                    className="w-full"
                  />
                  <div className="text-right text-sm font-bold text-blue-600">
                    {editingReview.ratingProfessionalism} 分
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingReview({ ...editingReview, ratingTeamwork: Number(e.target.value) })
                    }
                    className="w-full"
                  />
                  <div className="text-right text-sm font-bold text-blue-600">
                    {editingReview.ratingTeamwork} 分
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="p-6 border-t border-slate-100">
            ...
            <textarea
              ...
              onChange={(e) =>
                setEditingReview({ ...editingReview, managerComment: e.target.value })
              }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {reviewableUsers.map((user) => {
            ...
            const canEdit =
              ...
              (currentUser.role === 'SUPERVISOR' && user.id !== currentUser.id);

            return (
              <div
                key={user.id}
                className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between hover:shadow-md transition"
              >
