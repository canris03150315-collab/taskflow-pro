...
export const ChatSystem: React.FC<ChatSystemProps> = ({ currentUser, users, departments }) => {
  ...
  useEffect(() => {
    ...
    if (!isProduction) {
      // Enable WebSocket for HTTP connections
      const wsUrl =
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const newMsg = await api.chat.sendMessage(
        activeChannelId,
        currentUser.id,
        content,
        currentUser
  ...
  return (
    <div className="flex h-[calc(100vh-100px)] bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      {/* Sidebar */}
      <div
        className={`${activeChannelId ? 'hidden md:flex' : 'flex'} w-full md:w-80 border-r border-slate-200 flex-col bg-white`}
      >
        {/* Search Bar */}
          ...
          {sidebarMode === 'CHAT' ? (
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100 p-1 hover:bg-red-100 rounded transition text-red-500"
                          title="刪除聊天室"
                        >
                          🗑️
                        </button>
            ...
            <div className="p-4 space-y-4">
                        ...
                        >
                          ...
                          <div className="text-left flex-1">
                            ...
                            <div className="text-xs text-slate-500">
                              {channel.participants.length} 位成員
                            </div>
                          </div>
                        </button>
                      ))}
              ...
              <div className="space-y-4">
                {departments
                  .filter((dept) =>
                    users.some((u) => u.department === dept.id && u.id !== currentUser.id)
                  )
                  .map((dept) => {
                    const deptUsers = users.filter(
                      (u) => u.department === dept.id && u.id !== currentUser.id
                    );
          ...
          <>
            ...
            >
              {messages.map((msg, index) => {
                ...
                const showAvatar =
                  !isMe && (index === 0 || messages[index - 1].userId !== msg.userId);
                const showName =
                  !isMe && (index === 0 || messages[index - 1].userId !== msg.userId);
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`max-w-[65%] ${isMe ? 'items-end' : 'items-start'} flex flex-col`}
                    >
                      {showName && (
                        <span className="text-xs text-slate-600 mb-1 font-medium px-1">
                          {msg.userName}
                        </span>
                      )}
                      <div className="relative group/msg">
                        ...
                        {/* Quick Actions */}
                        <div
                          className={`absolute top-0 ${isMe ? 'right-full mr-2' : 'left-full ml-2'} opacity-0 group-hover/msg:opacity-100 transition-opacity flex gap-1`}
                        >
                          <button
            ...
            <form
              ...
              className="p-4 pb-20 md:pb-4 border-t border-slate-200 bg-white"
            >
              {replyToMessage && (
