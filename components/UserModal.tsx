...
export const UserModal: React.FC<UserModalProps> = ({ isOpen, onClose, onSubmit, currentUser, editingUser, departments }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (editingUser) {
        ...
        setDepartment(currentUser.role === Role.SUPERVISOR ? currentUser.department : departments[0]?.id || '');
        setAvatar(`https://api.dicebear.com/9.x/avataaars/svg?seed=${Math.random()}&backgroundColor=b6e3f4`);
        setPermissions([]);
      }
    }
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto p-6 space-y-6">
          ...
          <form id="user-form" onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div>
                    <label className="block text-sm font-bold text-slate-500 mb-1">
                      登入密碼 {editingUser && <span className="text-gray-400 text-xs">(留空則不修改)</span>}
                    </label>
                    <input
                      type="text"
            ...
            {/* Role */}
            <div>
              <label className="block text-sm font-bold text-slate-500 mb-1">職務角色 {isSelf && <span className="text-orange-400 text-xs">(無法變更自己)</span>}</label>
              <select
                value={role}
              ...
              >
                ...
                {!isBossOrManager && role !== Role.EMPLOYEE && (
            ...
            {/* Department (Dynamic) */}
            <div>
              <label className="block text-sm font-bold text-slate-500 mb-1">所屬部門 {isSelf && <span className="text-orange-400 text-xs">(無法變更自己)</span>}</label>
              <select
                value={department}
            ...
            {canEditSensitive && role !== Role.BOSS && (
                <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                    <label className="block text-sm font-bold text-slate-700 mb-3">特權設定 (進階授權)</label>
                    <div className="space-y-2">
                        {permissionOptions.map(option => {
                            // Hide SYSTEM_RESET if current user isn't Boss
                            if (option.value === 'SYSTEM_RESET' && currentUser.role !== Role.BOSS) return null;

                            return (
                              <label key={option.value} className={`flex items-center gap-2 cursor-pointer hover:bg-slate-100 p-1 rounded transition ${option.isDangerous ? 'text-red-600' : ''}`}>
                                  <input 
                                      type="checkbox" 
                                  ...
                                  />
                                  <span className={`text-sm ${option.isDangerous ? 'font-bold' : 'text-slate-600'}`}>{option.label}</span>
                              </label>
                            );
                        })}
                    </div>
