...
export const ForumView: React.FC<ForumViewProps> = ({
  ...
  const getDeptName = (id: string) =>
    departments.find((d) => d.id === id)?.name || (id === 'ALL' ? '公司全體 / 總經理室' : id);
  ...
  const getStatusBadge = (status: SuggestionStatus) => {
    switch (status) {
      ...
      case SuggestionStatus.REVIEWING:
        return (
          <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold border border-blue-200 flex items-center gap-1">
            <span className="animate-pulse w-2 h-2 bg-blue-500 rounded-full"></span> 審核中
          </span>
        );
  ...
  const canManageStatus = (suggestion: Suggestion) => {
    ...
    if (currentUser.role === Role.SUPERVISOR && suggestion.targetDeptId === currentUser.department)
      return true;
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20 animate-fade-in">
      ...
      <div className="space-y-6">
        ...
        {displayedSuggestions.map((suggestion) => {
          ...
          const displayAvatar =
            suggestion.isAnonymous && !isMe
              ? 'https://api.dicebear.com/9.x/avataaars/svg?seed=Secret&backgroundColor=e2e8f0'
              : author?.avatar ||
                `https://api.dicebear.com/9.x/avataaars/svg?seed=${encodeURIComponent(author?.name || 'unknown')}&backgroundColor=e2e8f4`;

          const displayName =
            suggestion.isAnonymous && !isMe ? '某位熱心同仁' : author?.name || '未知使用者';

          const displayRole =
            suggestion.isAnonymous && !isMe
              ? '秘密客'
              : author?.role === Role.EMPLOYEE
                ? '員工'
                : '管理層';
          ...
          return (
            <div
              key={suggestion.id}
              className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition duration-300"
            >
              {/* Card Header */}
              <div className="p-6 border-b border-slate-100 flex items-start justify-between gap-4">
                <div className="flex gap-4">
                  <div className="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 overflow-hidden flex-shrink-0">
                    <img
                      src={displayAvatar}
                      alt="Author"
                      className={`w-full h-full object-cover ${suggestion.isAnonymous && !isMe ? 'opacity-50 scale-75' : ''}`}
                    />
                  </div>
                  <div>
                ...
                {managerAccess && (
                  <select
                    ...
                    onChange={(e) =>
                      onUpdateStatus(suggestion.id, e.target.value as SuggestionStatus)
                    }
              ...
              <div className="bg-slate-50/50 p-6 border-t border-slate-100 space-y-4">
                {suggestion.comments.map((comment) => {
                  ...
                  return (
                    <div
                      key={comment.id}
                      className={`flex gap-3 text-sm ${isOfficial ? 'bg-blue-50/50 p-3 rounded-xl border border-blue-100' : ''}`}
                    >
                      <img
                        src={commenter?.avatar || defaultAvatar}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-2">
                            <span
                              className={`font-bold ${isOfficial ? 'text-blue-700' : 'text-slate-700'}`}
                            >
                              {commenter?.name || '未知用戶'}
                            </span>
                            {isOfficial && (
                ...
                <div className="flex gap-2 mt-4">
                  <input
                    ...
                    onChange={(e) =>
                      setCommentInputs((prev) => ({ ...prev, [suggestion.id]: e.target.value }))
                    }
                    placeholder="加入討論..."
