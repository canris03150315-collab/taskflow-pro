...
export const ChatMessageReadStatusModal: React.FC<ChatMessageReadStatusModalProps> = ({
  ...
  // Hooks 必須在 early return 之前，順序固定（React Rules of Hooks）
  ...
  const readUsers = allParticipants.filter(
    (u) => (message.readBy || []).includes(u.id) && u.id !== message.userId
  );

  // 未讀：應該在頻道裡，但 ID 不在 readBy 列表，且不是發送者
  const unreadUsers = allParticipants.filter(
    (u) => !(message.readBy || []).includes(u.id) && u.id !== message.userId
  );
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {readUsers.length === 0 && (
                <p className="text-xs text-slate-300 italic pl-3">尚無人閱讀</p>
              )}
              {readUsers.map((u) => (
                <div
                  key={u.id}
                  ...
                  >
                    <div className="relative">
                      <img
                        ...
                        className="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 grayscale"
                      />
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-bold text-slate-700 truncate">{u.name}</div>
