...
export const PersonnelView: React.FC<PersonnelViewProps> = ({
  ...
  const visibleUsers = useMemo(() => {
    ...
    if (currentUser.role === Role.SUPERVISOR) {
      ...
      result = result.filter(
        (u) => u.department === currentUser.department || u.department === 'UNASSIGNED'
      );
    }
  ...
  const TreeCard = ({ user, isRoot = false }: { user: User; isRoot?: boolean }) => (
    ...
    >
      ...
      <div className="text-center w-full">
        ...
        <span
          className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase inline-block mb-1 ${getRoleBadgeColor(user.role)}`}
        >
      ...
      {!(
        ...
        (user.role === Role.BOSS || user.role === Role.MANAGER)
      ) && (
        <div className="absolute top-1 right-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100 transition-opacity bg-white/90 rounded-lg shadow-sm border border-slate-100 flex">
          <button
  ...
  const renderTree = () => {
    // 1. Find Root (Boss)
    const rootUser =
      currentUser.role === Role.BOSS ? users.find((u) => u.role === Role.BOSS) : currentUser;
      ...
      return departments
        .map((dept) => {
          const supervisor = users.find(
            (u) => u.role === Role.SUPERVISOR && u.department === dept.id
          );
    ...
    const myDeptEmployees =
      currentUser.role === Role.SUPERVISOR ? getEmployees(currentUser.department) : [];
    ...
    return (
      <div className="flex flex-col items-center py-10 overflow-x-auto min-h-[600px] w-full bg-slate-50/50 rounded-xl">
        ...
        <div className="flex flex-col items-center relative z-20">
          ...
          {managers.length > 0 || branches?.length || myDeptEmployees.length ? (
        ...
        {currentUser.role === Role.BOSS &&
          managers.length === 0 &&
          branches &&
          branches.length > 0 && <div className="w-px h-0 bg-slate-300"></div>}
      ...
      {viewMode === 'DEPT_MGMT' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {visibleUsers.map((user) => (
            <div
              key={user.id}
              className={`rounded-xl border shadow-sm p-5 flex items-start gap-4 relative group hover:shadow-md transition ${getDeptColor(user.department)}`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${getRoleBadgeColor(user.role)}`}
                  >
                    {getRoleLabel(user.role)}
                  </span>
                </div>
              ...
              {!(
                ...
                <div className="absolute top-2 right-2 flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100 transition-opacity bg-white/80 backdrop-blur-sm p-1 rounded-lg">
                  <button
                    onClick={() => onEditUser(user)}
