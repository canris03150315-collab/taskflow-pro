...
export const ReportView: React.FC<ReportViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'worklogs' | 'reports' | 'audit' | 'platform'>(
    'worklogs'
  );
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const updatedContent = {
        ...
        conversionRate:
          editContent.lineLeads > 0
            ? Math.round((editContent.registrations / editContent.lineLeads) * 100)
            : 0,
        firstDepositRate:
          editContent.registrations > 0
            ? Math.round((editContent.firstDeposits / editContent.registrations) * 100)
            : 0,
      };

      await api.reports.update(editingReport.id, updatedContent);
      setReports(
        reports.map((r) => (r.id === editingReport.id ? { ...r, content: updatedContent } : r))
      );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20 animate-fade-in">
              ...
              <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg p-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-blue-900 font-medium">📅 顯示最近 7 天內的報表</p>
      ...
      {/* Tab Content */}
      {activeTab === 'worklogs' ? (
        <WorkLogTab currentUser={currentUser} departments={departments} users={users} />
      ) : activeTab === 'platform' ? (
                          ...
                          >
                            <div className="flex-1">
                              <div className="font-medium text-gray-900">
                                {pending.firstApproverName} 的審核請求
                              </div>
                              <div className="text-sm text-gray-600">
                                {pending.firstApproverDept} ·{' '}
                                {new Date(pending.firstApprovedAt).toLocaleString('zh-TW')}
                              </div>
                              <div className="text-xs text-gray-600 mt-1">
                                原因：{pending.firstApprovalReason}
                              </div>
                            </div>
                            <button
                              onClick={() => {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {users
                      .filter(
                        (user) => filterDepartment === 'all' || user.department === filterDepartment
                      )
                      .map((user) => (
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditContent({
                                      ...editContent!,
                                      lineLeads: Number(e.target.value),
                                    })
                                  }
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditContent({
                                      ...editContent!,
                                      registrations: Number(e.target.value),
                                    })
                                  }
                                  className="w-full p-2 border border-slate-300 rounded-lg"
                                />
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                              <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                                ...
                                <div className="text-xl font-bold text-slate-700">
                                  {report.content.lineLeads}
                                </div>
                              </div>
                              <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                                <div className="text-xs text-slate-400">註冊人數</div>
                                <div className="text-xl font-bold text-slate-700">
                                  {report.content.registrations}
                                </div>
                              </div>
                              <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                                <div className="text-xs text-slate-400">首充人數</div>
                                <div className="text-xl font-bold text-slate-700">
                                  {report.content.firstDeposits}
                                </div>
                              </div>
                              <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                                <div className="text-xs text-slate-400">淨入金額</div>
                                <div
                                  className={`text-xl font-bold ${report.content.netIncome >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                                >
                                  {formatCurrency(report.content.netIncome)}
                                </div>
                              </div>
                              ...
                              <div className="flex gap-6 text-sm">
                                <span className="text-slate-500">
                                  ...
                                  <span className="text-slate-800 font-medium">
                                    {formatCurrency(report.content.depositAmount)}
                                  </span>
                                </span>
                                <span className="text-slate-500">
                                  提現:{' '}
                                  <span className="text-slate-800 font-medium">
                                    {formatCurrency(report.content.withdrawalAmount)}
                                  </span>
                                </span>
                              </div>
                            </div>
