    ...
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto pb-20">
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 font-bold mt-1">公司重要政令宣導與活動通知</p>
        </div>
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4 pr-4 md:pr-16">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2.5 rounded-xl shadow-inner ${isImportant ? 'bg-red-50 text-red-500' : 'bg-slate-200 text-slate-500'}`}
                  >
                    {isImportant ? (
                      <svg
                        className="w-8 h-8"
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3
                        className={`text-xl font-bold ${!isRead ? 'text-slate-900' : 'text-slate-600'}`}
                      >
                        {ann.title}
                      </h3>
                      {!isRead && (
              ...
              <div
                className={`whitespace-pre-line leading-relaxed pl-1 md:pl-16 mb-6 text-sm ${!isRead ? 'text-slate-800 font-medium' : 'text-slate-500'}`}
              >
                {ann.content}
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canManage && (
                    ...
                    >
                      ...
                      查看已讀狀況 (
                      {Array.isArray(ann.readBy)
                        ? new Set(
                            ann.readBy.map((r: any) =>
                              typeof r === 'object' && r !== null
                                ? r.userId || r.id || JSON.stringify(r)
                                : String(r)
                            )
                          ).size
                        : 0}
