    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-slate-700 mb-2">AI 助理</h2>
          <p className="text-slate-500">此功能僅限主管以上使用</p>
        </div>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col bg-stone-50">
      ...
      <div className="bg-white px-6 py-4 flex items-center justify-between border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900">AI 智能助理</h1>
        ...
        {isLoadingHistory ? (
          ...
          <>
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-6 text-center mb-4">
              ...
              <h2 className="text-2xl font-bold text-slate-900 mb-2">您好，{currentUser.name}</h2>
            ...
            {alerts.length > 0 && (
              <div className="mb-4">
                <button
                  ...
                  className="flex items-center gap-2 text-sm font-semibold text-slate-700 mb-2 hover:text-indigo-600 transition"
                >
              ...
              messages.map((msg) => (
                ...
                >
                  {msg.role === 'assistant' ? (
                                ...
                                return (
                                  <div className="mt-3 space-y-1">
                                    {results.map((r: any, i: number) => (
                                      <div
                                        key={i}
                                        className={`flex items-center gap-2 text-xs p-1.5 rounded-lg ${r.success ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
                                      >
                                        {r.success ? (
                                        ...
                                        <span>
                                          {r.description || r.action}: {r.message}
                                        </span>
                                        {r.reportUrl && (
                                          <a
                                            href={r.reportUrl}
                    ...
                    <div className="max-w-[85%]">
                      <div className="bg-slate-200 rounded-2xl rounded-tr-sm px-4 py-3 group relative">
                        <p className="text-slate-900 text-sm leading-relaxed whitespace-pre-wrap break-words">
                          {msg.message}
                        </p>
                        <div className="text-xs text-slate-500 mt-2 text-right flex items-center justify-end gap-2">
                          <span>
                            {new Date(msg.created_at).toLocaleTimeString('zh-TW', {
                              hour: '2-digit',
                              minute: '2-digit',
            ...
            {pendingActions && pendingActions.length > 0 && (
              <div className="bg-white rounded-2xl shadow-lg border-2 border-indigo-200 p-4 mx-4 animate-fade-in">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-bold text-slate-800">確認執行以下操作</span>
                </div>
                ...
                <div className="space-y-2 mb-4">
                  {pendingActions.map((action, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className={`p-3 rounded-xl border ${isDanger ? 'bg-red-50 border-red-200' : 'bg-stone-50 border-slate-200'}`}
                      >
                        <div className="flex items-center gap-2">
                          <span
                ...
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    className="px-4 py-2 text-sm text-slate-600 bg-stone-100 hover:bg-slate-200 rounded-lg transition font-bold"
                  >
                    取消
                  </button>
                  <button
            ...
            {actionResults && !pendingActions && (
              <div className="bg-white rounded-2xl shadow-sm border border-green-200 p-4 mx-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-bold text-slate-800 text-sm">操作已執行</span>
                </div>
                <div className="space-y-1">
                  {actionResults.map((r, i) => (
                    <div
                      key={i}
                      className={`flex items-center gap-2 text-sm p-2 rounded-lg ${r.success ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
                    >
      ...
      <div className="bg-white border-t border-slate-100 px-4 py-4">
        <div className="flex items-center gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 bg-stone-100 rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
            disabled={isLoading}
          />
          <button
...
function formatMessage(text: string): React.ReactNode {
  ...
  lines.forEach((line, i) => {
    ...
    if (line.startsWith('### ')) {
      elements.push(
        <div key={i} className="font-bold text-base mt-3 mb-1">
          {line.slice(4)}
        </div>
      );
    } else if (line.startsWith('## ')) {
      elements.push(
        <div key={i} className="font-bold text-lg mt-3 mb-1">
          {line.slice(3)}
        </div>
      );
    } else if (line.startsWith('# ')) {
      elements.push(
        <div key={i} className="font-bold text-xl mt-3 mb-1">
          {line.slice(2)}
        </div>
      );
