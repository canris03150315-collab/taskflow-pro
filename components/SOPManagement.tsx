            ...
            <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {editIsDaily ? '員工每天需要勾選完成各項任務' : '員工只需閱讀並確認已讀'}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-2">
                {editItems.map((item, idx) => (
                  <div key={idx} className="flex gap-2 items-start">
                    <span className="text-slate-400 py-2 text-sm font-mono w-6 text-right">
                      {idx + 1}.
                    </span>
                    <textarea
                      value={item}
      ...
      <div className="space-y-4">
        {visibleTemplates.length === 0 ? (
          ...
          visibleTemplates.map((tpl) => (
            <div
              key={tpl.id}
              className="flex flex-col md:flex-row md:items-center justify-between p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md transition group gap-4"
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h4 className="font-bold text-slate-800 text-lg truncate">{tpl.title}</h4>
                  <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded font-bold text-slate-500 border border-slate-200 whitespace-nowrap">
                    {getDeptName(tpl.departmentId)}
                  </span>
                </div>
              ...
              <div className="flex gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                <button
                  ...
                  className="text-slate-500 font-bold text-xs hover:bg-slate-100 px-3 py-1.5 rounded transition border border-slate-200 flex items-center gap-1"
                  title="複製文件"
                >
                  <svg
                ...
                <button
                  ...
                  className="text-blue-600 font-bold text-xs hover:bg-blue-50 px-3 py-1.5 rounded transition border border-blue-100 flex items-center gap-1"
                >
                  <svg
                    className="w-3.5 h-3.5"
                    fill="none"
                ...
                <button
                  ...
                  className="text-red-500 font-bold text-xs hover:bg-red-50 px-3 py-1.5 rounded transition border border-red-100 flex items-center gap-1"
                >
                  <svg
                    className="w-3.5 h-3.5"
                    fill="none"
