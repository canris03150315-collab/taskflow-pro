...
const KOLManagementView = lazy(() =>
  import('./KOLManagementView').then((m) => ({ default: m.KOLManagementView }))
);
...
export const FinanceView: React.FC<FinanceViewProps> = ({
  ...
  const canAddRecord = useMemo(() => {
    ...
    if (activeTab === 'DEPT') {
      ...
      const targetDept = isBoss ? filterDept : currentUser.department;
      if (
        currentUser.department === targetDept &&
  ...
  const filteredModalUsers = useMemo(() => {
    ...
    return modalAvailableUsers.filter(
      (u) =>
  ...
  const renderDeleteButton = (record: FinanceRecord) => {
    ...
    return (
      <button
        onClick={async (e) => {
          e.stopPropagation();
          if (await showConfirm('確定要刪除這筆紀錄嗎？')) onDeleteRecord(record.id);
        }}
        ...
        <>
              ...
              {activeTab === 'PERSONAL' && (
                <>
                  ...
                  {isBoss || isSupervisor ? (
                    ...
                    >
                      {users
                        .filter((u) => isBoss || u.department === currentUser.department)
                        .map((u) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group col-span-1 md:col-span-2">
              <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition text-6xl">
                💰
              </div>
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {displayedRecords.length === 0 ? (
                  ...
                  displayedRecords.map((record) => (
                    <tr
                      key={record.id}
                      className={`hover:bg-slate-50/50 transition ${record.status === 'PENDING' ? 'bg-amber-50/30' : ''}`}
                    >
                          ...
                          <div className="text-[10px] text-slate-400">
                            {record.scope === 'DEPARTMENT' ? (
                              <span className="text-blue-600 font-bold">
                                {getDeptName(record.departmentId)}公費
                              </span>
                            ) : (
                      ...
                      <td
                        className={`p-4 text-right font-mono font-bold text-base ${record.type === 'INCOME' ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {record.type === 'INCOME' ? '+' : '-'}
            ...
            <div className="md:hidden p-4 space-y-4 bg-slate-50/50">
              {displayedRecords.length === 0 ? (
                ...
                displayedRecords.map((record) => (
                  <div
                    key={record.id}
                    className={`bg-white p-4 rounded-xl border shadow-sm ${record.status === 'PENDING' ? 'border-amber-200 bg-amber-50/20' : 'border-slate-200'}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div
                        className={`font-mono font-black text-lg ${record.type === 'INCOME' ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {record.type === 'INCOME' ? '+' : '-'}
                        {record.amount.toLocaleString()}
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      ...
                      {record.scope === 'DEPARTMENT' ? (
                        ...
                        <span className="text-indigo-600 text-xs font-bold bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                          {getUserName(record.ownerId)}
                        </span>
                      )}
                      {record.status === 'PENDING' && (
                        <span className="text-amber-600 text-xs font-bold bg-amber-100 px-2 py-0.5 rounded border border-amber-200 animate-pulse">
                          待確認
                        </span>
                      )}
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                      ...
                      >
                        {selectedTargetUser ? (
                          <div className="flex items-center gap-2">
                            <img
                              src={
                                selectedTargetUser.avatar ||
                                `https://api.dicebear.com/9.x/avataaars/svg?seed=${encodeURIComponent(selectedTargetUser.name || 'default')}`
                              }
                            ...
                            <span className="text-xs text-slate-400">
                              ({getDeptName(selectedTargetUser.department)})
                            </span>
                          </div>
                        ) : (
                            ...
                            {filteredModalUsers.map((u) => (
                              ...
                              >
                                <img
                                  src={
                                    u.avatar ||
                                    `https://api.dicebear.com/9.x/avataaars/svg?seed=${encodeURIComponent(u.name || 'default')}`
                                  }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 font-mono text-lg"
                      placeholder="0"
                    />
                  </div>
                  ...
                  <div className="pt-4 flex justify-end gap-2">
                    ...
                    <button
                      type="submit"
                      className="px-6 py-2 bg-slate-800 text-white font-bold rounded-lg hover:bg-slate-700 shadow-md"
                    >
                      確認儲存
                    </button>
                  </div>
          ...
          {viewImage && (
            ...
            >
              <div className="relative max-w-4xl max-h-[90vh]">
                ...
                <button
                  onClick={() => setViewImage(null)}
                  className="absolute -top-4 -right-4 bg-white rounded-full p-2 text-black hover:scale-110 transition shadow-lg"
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
