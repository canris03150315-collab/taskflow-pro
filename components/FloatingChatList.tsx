...
export const FloatingChatList: React.FC<FloatingChatListProps> = ({
  ...
  const recentChats = useMemo(() => {
    return channels
      ...
      .sort((a, b) => {
        ...
        return (
          new Date(b.channel.lastMessage.timestamp).getTime() -
  ...
  return (
    <>
      ...
      {/* 聊天列表彈窗 */}
      <div className="fixed bottom-safe-24 right-6 z-50 w-[calc(100vw-3rem)] max-w-xs sm:max-w-sm bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-slide-up">
        {/* 標題列 */}
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {channel.lastMessage && (
                              <p
                                className={`text-sm truncate ${unreadCount > 0 ? 'text-slate-700 font-medium' : 'text-slate-500'}`}
                              >
                ...
                allContacts.map((user) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {/* 聊天圖標 */}
                      <svg
                        className="w-5 h-5 text-slate-400 group-hover:text-blue-600 transition"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
