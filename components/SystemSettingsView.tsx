...
export const SystemSettingsView: React.FC<SystemSettingsViewProps> = ({
  ...
  const [draggedItem, setDraggedItem] = useState<{ groupIndex: number; itemIndex: number } | null>(
    null
  );
  const [hasChanges, setHasChanges] = useState(false);
  const [avatar, setAvatar] = useState(
    currentUser.avatar ||
      `https://api.dicebear.com/9.x/avataaars/svg?seed=${encodeURIComponent(currentUser.name || 'default')}&backgroundColor=b6e3f4`
  );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20 animate-fade-in">
      ...
      {showDataCleanup && <DataCleanupView onClose={() => setShowDataCleanup(false)} />}
    </div>
  );
};
