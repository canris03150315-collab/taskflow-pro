...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const { type, amount, description, category, date, departmentId } = req.body;
    ...
    dbCall(db, 'prepare', 
      'INSERT INTO finance (id, type, amount, description, category, user_id, department_id, date, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(
      id, 
      type, 
      amount, 
