...
console.log('=== Creating LeaveRequestService ===');
...
const serviceContent = `class LeaveRequestService {
  ...
  static async approveLeaveRequest(db, id, reviewerId, allowConflict = false) {
    const now = new Date().toISOString();
    ...
    await db.run(
      \`UPDATE leave_requests
       ...
       WHERE id = ?\`,
      ['APPROVED', reviewerId, now, now, id]
    );
  ...
  static async rejectLeaveRequest(db, id, reviewerId, rejectReason) {
    const now = new Date().toISOString();
    ...
    await db.run(
      \`UPDATE leave_requests
       ...
       WHERE id = ?\`,
      ['REJECTED', reviewerId, now, rejectReason, now, id]
    );
...
console.log('+ LeaveRequestService created at:', filePath);
