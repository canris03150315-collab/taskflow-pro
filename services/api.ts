...
const RealApi = {
  auth: {
    login: async (username: string, password: string): Promise<User | undefined> => {
      const res = await request<{ user: User; token: string }>('POST', '/auth/login', {
        username,
        password,
      });
      if (res.token) {
    ...
    changePassword: (userId: string, currentPassword: string, newPassword: string) =>
      request<{ success: boolean }>('POST', `/users/${userId}/change-password`, {
        currentPassword,
        newPassword,
      }),
  },
  ...
  departments: {
    ...
    create: async (dept: DepartmentDef) => {
      const response = await request<{ department: DepartmentDef; message: string }>(
        'POST',
        '/departments',
        dept
      );
  ...
  announcements: {
    ...
    update: (id: string, ann: Partial<Announcement>) =>
      request<Announcement>('PUT', `/announcements/${id}`, ann),
    delete: (id: string) => request<void>('DELETE', `/announcements/${id}`),
    markRead: (id: string, userId: string) =>
      request<void>('POST', `/announcements/${id}/read`, { userId }),
  },
  reports: {
    ...
    approval: {
      checkStatus: async (
        authId: string
      ): Promise<{ isAuthorized: boolean; authorization: any | null }> => {
        try {
          return await request<{ isAuthorized: boolean; authorization: any | null }>(
            'GET',
      ...
      getAuditLog: async (params: {
        ...
        try {
          ...
          return await request<{ logs: any[]; total: number }>(
            'GET',
            `/reports/approval/audit-log${qs ? '?' + qs : ''}`
          );
        } catch (error) {
  ...
  forum: {
    ...
    addComment: (suggestionId: string, content: string, isOfficial: boolean) =>
      request<{ comment: any }>('POST', `/forum/${suggestionId}/comments`, {
        content,
  ...
  attendance: {
    getTodayStatus: async (userId: string): Promise<AttendanceRecord | null> => {
      try {
        ...
        return {
          ...
          clockoutMissing: Boolean(record.clockout_missing),
    ...
    getHistory: async (): Promise<AttendanceRecord[]> => {
      ...
      return (response.records || []).map((r: any) => ({
        ...
        status: r.status,
        clockoutMissing: Boolean(r.clockout_missing),
    ...
    manualEntry: async (
      ...
      const response = await request<{ success: boolean; mode: string; record: any }>(
        'POST',
        '/attendance/manual',
        {
          userId,
    ...
    updateManualEntry: async (
      ...
      const response = await request<{ success: boolean; record: any }>(
        'PUT',
        `/attendance/manual/${id}`,
        {
          clockIn,
          clockOut,
          reason,
  ...
  chat: {
    getChannels: async (userId: string): Promise<ChatChannel[]> => {
      try {
        ...
        return (response.channels || []).map((ch: any) => ({
          ...
          lastMessage:
            ch.lastMessage || ch.last_message
              ? {
                  id: (ch.lastMessage || ch.last_message).id,
                  channelId:
                    (ch.lastMessage || ch.last_message).channel_id ||
                    (ch.lastMessage || ch.last_message).channelId,
                  userId:
                    (ch.lastMessage || ch.last_message).user_id ||
                    (ch.lastMessage || ch.last_message).userId,
                  userName:
                    (ch.lastMessage || ch.last_message).user_name ||
                    (ch.lastMessage || ch.last_message).userName,
                  ...
                  readBy:
                    (ch.lastMessage || ch.last_message).read_by ||
                    (ch.lastMessage || ch.last_message).readBy ||
                    [],
                }
              : undefined,
    ...
    getMessages: async (
      ...
      const response = await request<{ messages: any[]; hasMore?: boolean }>(
        'GET',
        `/chat/channels/${channelId}/messages${queryString ? `?${queryString}` : ''}`
      );
      return {
    ...
    sendMessage: async (
      ...
      const response = await request<{ message: any }>(
        'POST',
  ...
  schedules: {
    ...
    approve: async (id: string, data: any) =>
      request<any>('POST', `/schedules/${id}/approve`, data),
    ...
    updateRules: async (departmentId: string, rules: any) =>
      request<any>('PUT', `/schedules/rules/${departmentId}`, rules),
  },
  workLogs: {
  ...
  kol: {
    ...
    getKolPayments: async (
      ...
      return request<{ payments: any[]; total: number }>(
        'GET',
        `/kol/profiles/${kolId}/payments${qs ? '?' + qs : ''}`
      );
    },
    createKolPayment: async (data: {
      kolId: string;
      amount: number;
      paymentDate: string;
      notes?: string;
  ...
  aiAssistant: {
    ...
    sendQuery: async (message: string): Promise<any> => {
      ...
      return request<any>('POST', isCentral ? '/central/super-ai/query' : '/ai-assistant/query', {
        message,
      });
    },
    confirmAction: async (pendingActionId: string): Promise<any> => {
      ...
      return request<any>(
        'POST',
        isCentral ? '/central/super-ai/confirm' : '/ai-assistant/confirm',
        { pendingActionId }
      );
    },
    cancelAction: async (pendingActionId: string): Promise<any> => {
      const isCentral = (import.meta as any).env?.VITE_INSTANCE_MODE === 'central';
      return request<any>('POST', isCentral ? '/central/super-ai/cancel' : '/ai-assistant/cancel', {
        pendingActionId,
      });
    },
    getAlerts: async (): Promise<{ alerts: any[] }> => {
      ...
      return request<{ alerts: any[] }>(
        'GET',
        isCentral ? '/central/super-ai/alerts' : '/ai-assistant/alerts'
      );
    },
  ...
  files: {
    ...
    async listOperations(filters: any = {}): Promise<any[]> {
      ...
      const res = await request<{ items: any[] }>(
        'GET',
        `/files/operations/list?${params.toString()}`
      );
      return res.items || [];
    },
  },
